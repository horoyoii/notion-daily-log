...
2. **템플릿 복제**: Notion API를 통해 템플릿 페이지 복제 (하위 페이지 자동 포함)
3. **날짜 설정**: 복제된 페이지의 제목과 작성일을 오늘 날짜로 업데이트
4. **중복 확인**: 동일한 날짜의 로그가 이미 존재하면 생성 건너뜀
...
- Notion API의 `duplicate` 엔드포인트는 하위 페이지를 자동으로 복제합니다.
- 문제가 있다면 템플릿 페이지의 권한을 확인하세요.
